...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Byte-session realtime inference (live capture mode)"
    )
    parser.add_argument("--mode", choices=["live"], default="live")
    parser.add_argument("--source", default=r"\Device\NPF_{94B4B764-8E09-485A-9EF1-10C26641DF79}", help="network interface name")
    parser.add_argument(
        "--model-path",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="live mode duration in seconds; 0 runs until Ctrl+C",
    )
...
def main():
    ...
    run_single_pipeline(
        ...
        detector_json_path=args.detector_json,
