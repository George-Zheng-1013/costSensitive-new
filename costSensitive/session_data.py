...
def iter_with_progress(
    iterable: Iterable, total: Optional[int], desc: str, enable: bool
):
    if not enable:
        return iterable
    try:
...
def build_session_manifest(
    ...
    print(
        f"[INFO] start session build | captures={len(captures)} num_packets={num_packets} packet_len={packet_len}"
    )
    ...
    processed_files = 0
    skipped_by_cap = 0
    capture_loop = iter_with_progress(
        captures,
        total=len(captures),
        desc="extract-captures",
        enable=show_progress,
    )
    for file_path, rel_path, _domain, label in capture_loop:
        processed_files += 1
        ...
        if show_progress and processed_files % 20 == 0:
            print(
                "[PROGRESS] files={}/{} total_samples={} skipped_by_cap={}".format(
                    processed_files,
                    len(captures),
                    len(rows),
                    skipped_by_cap,
    ...
    print(
        "[DONE] manifest={} total_samples={} processed_files={} skipped_by_cap={} elapsed={:.2f}s".format(
            manifest_path,
            len(rows),
            processed_files,
            skipped_by_cap,
...
def parse_args():
    ...
    parser.add_argument(
        "--no-progress",
        action="store_true",
        help="disable tqdm/file progress output",
    )
